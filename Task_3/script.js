...
addtodobtn.onclick=()=>{
    ...
    const todotime = datetime();
    if(editid != null){
        todos.splice(editid,1,{'name':todolist,'time':todotime})
        editid = null;
    }
    else{
    todos.push({'name':todolist,'time':todotime});
    }
...
function display()
{
    ...
    todos.forEach((listodo,i) => {
     ...
     <button style="font-size: 16px;">${listodo.time}</button>
    ...
    comtodo.forEach((completetodo,i) => {
     ...
     <button style="font-size: 16px;">${completetodo.time}</button>
    </li>`
    });
...
function checkbtn(id){
    transid = id;
    const comtodotime = datetime();
    comtodo.push({'name':todos[id].name,'time':comtodotime});
    delbtn(transid);
    let statement1='';
    comtodo.forEach((completetodo,i) => {
        statement1 += `<li>${completetodo.name}
